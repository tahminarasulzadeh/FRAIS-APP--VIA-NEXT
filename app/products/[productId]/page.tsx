...
interface Product {
  ...
  price: number;
  image: string;
  popularity?: number;  // Make optional
  featureOne?: string;  // Make optional
...
export default function ProductDetails({ params }: { params: Promise<{ productId: string }> }) {
  ...
  if (!product) return <p className='text-xl text-center '>Loading...</p>;  // Loading state or message
    ...
    <div className="p-4">
      <div className="flex gap-5 flex-col items-center lg:flex-row justify-center">
        ...
        <div className="flex w-[450px] gap-[10px] lg:p-0 p-20 flex-col">
          ...
          <p className="text-gray-500 text-xl font-thin">{product.description}</p>
          ...
          <div className="border-[1px] group flex items-center justify-evenly cursor-pointer w-[70px] py-1 border-gray-500 bg-transparent">
            ...
            <div className="flex opacity-0 group-hover:opacity-100 flex-col gap-[5px] transition-all duration-500 ease-in">
              ...
              <IoIosArrowDown className="text-gray-500 text-sm font-thin" onClick={() => quantity > 1 && setQuantity(quantity - 1)} />
            </div>
          </div>
          <button
            className="w-full p-2 mt-3 border-[1px] bg-white border-black hover:text-white hover:bg-[#5E5E4A] transition-all duration-700 ease-in"
            onClick={() => {
              dispatch(addToCart( { ...product}))  // Dispatch with quantity
            }}
          >
            Add to Cart
          </button>

          <button className="w-full p-2 mt-3 border-[1px] hover:bg-white hover:border-black hover:text-[#5E5E4A] text-white bg-[#5E5E4A] transition-all duration-700 ease-in">
            Buy Now
          </button>
